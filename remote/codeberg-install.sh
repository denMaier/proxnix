#!/bin/bash
# codeberg-install.sh — curl-friendly wrapper for proxnix install.sh
#
# Intended usage:
#   bash -c "$(curl -fsSL https://.../remote/codeberg-install.sh)" -- [install.sh args...]
#
# Set the defaults below to match your own Codeberg repository before publishing
# this wrapper, or override them at runtime with:
...
: "${PROXNIX_REPO_HOST:=https://codeberg.org}"
: "${PROXNIX_REPO_OWNER:=CHANGEME_CODEBERG_OWNER}"
...
usage() {
  printf '%s\n' \
    ...
    "" \
    "Examples:" \
    "  bash -c \"\$(curl -fsSL https://.../remote/codeberg-install.sh)\"" \
    "  bash -c \"\$(curl -fsSL https://.../remote/codeberg-install.sh)\" -- --dry-run" \
    "" \
    "Repository overrides:" \
    "  PROXNIX_REPO_ARCHIVE_URL=https://codeberg.org/<owner>/<repo>/archive/<branch>.tar.gz" \
...
resolve_archive_url() {
  ...
  if [[ "$PROXNIX_REPO_OWNER" == CHANGEME_* ]]; then
    die "set PROXNIX_REPO_ARCHIVE_URL or edit ${SELF_NAME} with your Codeberg repo owner/name"
  fi
...
main() {
  ...
  exec "$bash_bin" "${repo_dir}/install.sh" "$@"
}
