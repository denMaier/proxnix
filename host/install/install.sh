...
do_systemd_timer() {
    ...
    do_install "$RUNTIME_DIR/systemd/${name}.timer"   "$SYSTEMD_UNIT_DIR/${name}.timer"   "644"
...
do_install "$RUNTIME_DIR/lxc/config/nixos.common.conf"  "$LXC_CONFIG_DIR/nixos.common.conf"
do_install "$RUNTIME_DIR/lxc/config/nixos.userns.conf"  "$LXC_CONFIG_DIR/nixos.userns.conf"
...
do_install "$RUNTIME_DIR/lxc/hooks/nixos-proxnix-prestart" \
           "$LXC_HOOKS_DIR/nixos-proxnix-prestart" "755"
do_install "$RUNTIME_DIR/lxc/hooks/nixos-proxnix-mount" \
           "$LXC_HOOKS_DIR/nixos-proxnix-mount" "755"
do_install "$RUNTIME_DIR/lxc/hooks/nixos-proxnix-poststop" \
...
do_install "$RUNTIME_DIR/nix/common.nix"        "$NIXLXC_DIR/common.nix"
do_install "$RUNTIME_DIR/nix/security-policy.nix" "$NIXLXC_DIR/security-policy.nix"
do_install "$RUNTIME_DIR/nix/configuration.nix" "$NIXLXC_DIR/configuration.nix"
