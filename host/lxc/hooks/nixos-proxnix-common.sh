...
proxnix_prepare_stage_base_dir() {
    mkdir -p "$PROXNIX_STAGE_BASE_DIR"
    chmod 0711 "$PROXNIX_STAGE_BASE_DIR"
}
...
proxnix_group_priv_dir() {
    local group="$1"
    printf '%s/groups/%s' "$NIXLXC_PRIV_DIR" "$group"
}
...
proxnix_group_secret_store_path() {
    local group="$1"
    printf '%s/secrets.sops.yaml' "$(proxnix_group_priv_dir "$group")"
}

proxnix_container_secret_groups_file_path() {
    local vmid="$1"
    printf '%s/secret-groups.list' "$(proxnix_container_dir "$vmid")"
}
...
proxnix_read_container_secret_groups() {
    ...
    path="$(proxnix_container_secret_groups_file_path "$vmid")"
    [[ -f "$path" ]] || return 0
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        proxnix_valid_secret_group_name "$line" \
            || { log "ERROR: invalid secret group name in ${path}: ${line}"; return 1; }
        printf '%s\n' "$line"
