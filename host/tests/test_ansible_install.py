...
class AnsibleInstallTests(unittest.TestCase):
    def test_ansible_playbook_installs_reconciler_runtime(self) -> None:
        ...
        self.assertIn('exec "$PROXNIX_HOST_BIN" hook prestart "$@"', prestart)
        ...
        self.assertIn('exec "$PROXNIX_HOST_BIN" hook mount "$@"', mount)
        ...
        self.assertIn('exec "$PROXNIX_HOST_BIN" hook poststop "$@"', poststop)
        rust_host = (ROOT / "host" / "rust" / "src" / "main.rs").read_text(
            encoding="utf-8"
        )
        self.assertIn('Some("prestart") => hook_prestart_main', rust_host)
        self.assertIn('Command::new("proxnix-reconcile-build-golden")', rust_host)
