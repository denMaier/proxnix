...
let
  ...
  installPythonSources = ''
    ...
    cp -a "$src/cli/src" "$out/share/proxnix-workstation/src"
    install -m 644 "$src/cli/pyproject.toml" "$out/share/proxnix-workstation/pyproject.toml"
    install -m 644 "$src/cli/uv.lock" "$out/share/proxnix-workstation/uv.lock"
  '';
      ...
      installPhase =
        let
          scripts =
            ...
            ++ lib.optionals withTui [ "cli/bin/proxnix-tui" ];
          wrappedScripts =
            [
              "proxnix"
              "proxnix-publish"
