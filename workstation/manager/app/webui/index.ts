...
import { createManagerRequestHandlers } from "../shared/capabilities/managerHandlers";
...
const FRONTEND_ROOT = new URL("./shared/frontend/", SRC_ROOT);
...
function printUsage(): void {
  console.log(`Usage:
  bun run app/webui/index.ts [--host <host>] [--port <port>]
...
async function buildMainviewBundle(): Promise<string> {
  ...
  const result = await Bun.build({
    entrypoints: [new URL("./shared/frontend/web.ts", SRC_ROOT).pathname],
    outdir,
    target: "browser",
    format: "esm",
    sourcemap: "linked",
