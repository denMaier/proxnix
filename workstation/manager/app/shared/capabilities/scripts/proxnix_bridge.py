...
def _repo_root() -> Path | None:
    script_path = Path(__file__).resolve()
    for parent in script_path.parents:
        if (parent / "workstation" / "cli" / "src" / "proxnix_workstation").is_dir():
            return parent
    return None
...
def _pythonpath_entries() -> list[str]:
    ...
    if repo_root is not None:
        repo_python = repo_root / "workstation" / "cli" / "src"
...
def _repo_cli(script_name: str) -> Path | None:
    ...
    cli_path = workstation_dir / "cli" / "bin" / script_name
    if cli_path.is_file():
        return cli_path
    return None


def _configured_cli(script_name: str) -> Path | None:
    scripts_dir = os.environ.get("PROXNIX_SCRIPTS_DIR", "").strip()
    config_path = default_config_path()
    if config_path.is_file():
        try:
            scripts_dir = parse_config_lines(config_path.read_text(encoding="utf-8")).get(
                "PROXNIX_SCRIPTS_DIR", scripts_dir
            )
        except ValueError:
            pass
    ...
    home = Path(os.environ.get("HOME", str(Path.home()))).expanduser()
    cli_path = Path(_expand_home_string(scripts_dir, home)) / script_name
    if cli_path.is_file():
        return cli_path
    return None
