...
class OrbSiteCliTests(unittest.TestCase):
    def test_prepare_local_site_tree_keeps_targeted_container_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (source_site / "private" / "shared" / "secrets.proxnix.json").parent.mkdir(parents=True, exist_ok=True)
            (source_site / "private" / "shared" / "secrets.proxnix.json").write_text("shared: value\n", encoding="utf-8")
            (source_site / "private" / "groups" / "g1" / "secrets.proxnix.json").parent.mkdir(parents=True, exist_ok=True)
            (source_site / "private" / "groups" / "g1" / "secrets.proxnix.json").write_text("group: value\n", encoding="utf-8")
            (source_site / "private" / "containers" / "132" / "secrets.proxnix.json").parent.mkdir(parents=True, exist_ok=True)
            (source_site / "private" / "containers" / "132" / "secrets.proxnix.json").write_text("container: value\n", encoding="utf-8")
            (source_site / "private" / "containers" / "999" / "secrets.proxnix.json").parent.mkdir(parents=True, exist_ok=True)
            (source_site / "private" / "containers" / "999" / "secrets.proxnix.json").write_text("other: value\n", encoding="utf-8")
            (source_site / "private" / "host_relay_identity.age").write_text("identity: value\n", encoding="utf-8")
            ...
            self.assertTrue((target_site / "private" / "shared" / "secrets.proxnix.json").is_file())
            self.assertTrue((target_site / "private" / "groups" / "g1" / "secrets.proxnix.json").is_file())
            self.assertTrue((target_site / "private" / "containers" / "132" / "secrets.proxnix.json").is_file())
            self.assertFalse((target_site / "private" / "containers" / "999").exists())
            self.assertTrue((target_site / "private" / "host_relay_identity.age").is_file())
    ...
    def test_remote_site_script_uses_prestart_and_persistent_service(self) -> None:
        ...
        self.assertIn('PRESTART_HOOK="${REPO_ROOT}/host/runtime/lxc/hooks/nixos-proxnix-prestart"', rendered)
